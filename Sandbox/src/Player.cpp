...
{
	m_ShipTexture = Spydr::Texture2D::Create("assets/textures/Ship.png");
}

void Player::OnUpdate(Spydr::Timestep ts)
{
	m_Time += ts;

	if (Spydr::Input::IsKeyPressed(SP_KEY_SPACE))
	{
...
void Player::OnRender()
{
	...
	Spydr::Renderer2D::DrawRotatedQuad({ m_Position.x, m_Position.y, 0.5f }, glm::radians(GetRotation()), { 1.0f, 1.3f }, m_ShipTexture);
}
